...
## V24 plan-prefix generic-label ablation

Prompt-only proof skeleton with generic `plan_` labels. These labels are not Lean hypotheses and are not theorem identifiers. Use them only to choose the proof shape; do not emit the label text as code.

- `plan_tree_induction`: induct on the `NatTree` argument so the two subtree induction hypotheses are available in the node case.
- `plan_leaf_simplification`: in the leaf case, simplify by unfolding the local tree and list metric definitions.
- `plan_node_split`: in the node case, split on whether the node value equals the target before simplifying keep/drop/count behavior.
- `plan_append_filter_simplification`: after the split, simplify mirror, flatten, keep/drop filters, and list metrics with the append/filter helper hypotheses and both subtree induction hypotheses.
- `plan_nat_add_normalization`: normalize natural-number addition with associativity, commutativity, and left-commutativity.
- `plan_packer_finish`: finish any remaining conjunction packaging with the available pair/top or named metric packers.

Do not emit these labels as theorem assumptions; they are not in scope for v24. The final answer must still be only the Lean proof body for `{{PROOF}}` and must pass Lean unchanged.
