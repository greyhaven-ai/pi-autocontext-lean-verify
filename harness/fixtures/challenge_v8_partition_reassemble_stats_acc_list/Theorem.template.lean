...
/-- Challenge v8 diagnostic: list-only partition/reassemble equivalence for the three-field stats accumulator. -/
theorem partitionReassembleStatsAcc_list
    (target : Nat) (xs : List Nat) (hits total sum : Nat) :
    statsAcc target (appendSlow (keepEqSlow target xs) (dropEqSlow target xs)) hits total sum =
      statsAcc target xs hits total sum := by
{{PROOF}}
