...
/-- Challenge v15: reassemble length/count/sum metric-order conclusion from metric-grouped hypotheses. -/
theorem orderLengthCountSum_from_metric_hyps (target : Nat) (tree : NatTree)
    (hLength : lengthSlow (keepEqSlow target (flatten (mirror tree))) =
