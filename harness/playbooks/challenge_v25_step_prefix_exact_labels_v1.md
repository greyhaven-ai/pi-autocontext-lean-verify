...
### challenge_v6_stats_acc_rev

- definitions: appendSlow, revSlow, lengthSlow, sumSlow, countSlow, statsAcc
- notable lemmas/tactics: induction, simp, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm, Nat.succ

```lean
have append_single : ∀ as x hits total sum,
    statsAcc target (appendSlow as [x]) hits total sum =
      statsAcc target as ((if x = target then 1 else 0) + hits) (Nat.succ total) (x + sum) := by
  intro as
  induction as with
  | nil =>
      intro x hits total sum
      simp [appendSlow, statsAcc]
  | cons a as ih =>
      intro x hits total sum
      simp [appendSlow, statsAcc, ih, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
induction xs generalizing hits total sum with
| nil =>
    simp [revSlow, statsAcc]
| cons x xs ih =>
    simp [revSlow, statsAcc, append_single, ih]
```

### challenge_v6_tree_partition_mirror

- definitions: appendSlow, lengthSlow, countSlow, keepEqSlow, dropEqSlow, mirror, flatten
- notable lemmas/tactics: induction, simp, constructor, congrArg, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm, Nat.succ
...
have count_keep : ∀ xs : List Nat,
  ...
  | cons x xs ih =>
      ...
      · simp [keepEqSlow, countSlow, h, ih]
      · simp [keepEqSlow, countSlow, h, ih]
have partition_length : ∀ xs : List Nat,
    lengthSlow (keepEqSlow target xs) + lengthSlow (dropEqSlow target xs) = lengthSlow xs := by
  intro xs
  induction xs with
  | nil =>
      simp [keepEqSlow, dropEqSlow, lengthSlow]
  | cons x xs ih =>
      by_cases h : x = target
      · simpa [keepEqSlow, dropEqSlow, lengthSlow, h, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using congrArg Nat.succ ih
      · simpa [keepEqSlow, dropEqSlow, lengthSlow, h, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using congrArg Nat.succ ih
have count_mirror : ∀ tree : NatTree,
    countSlow target (flatten (m
-- truncated in playbook
```

### challenge_v6_mapSucc_rev_count_sum

- definitions: appendSlow, revSlow, lengthSlow, sumSlow, countSlow, mapSuccSlow
- notable lemmas/tactics: induction, simp, constructor, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm, Nat.succ
...
have count_rev : ∀ (t : Nat) (as : List Nat), countSlow t (revSlow as) = countSlow t as := by
  ...
  | cons a as ih =>
      simp [revSlow, countSlow, append_count, ih, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
have count_mapSucc : ∀ (t : Nat) (as : List Nat),
    countSlow (Nat.succ t) (mapSuccSlow as) = countSlow t as := by
  intro t as
  induction as with
  | nil =>
      simp [mapSuccSlow, countSlow]
  | cons a as ih =>
      simp [mapSuccSlow, countSlow, ih]
have append_sum : ∀ as bs : List Nat,
    sumSlow (appendSlow as bs) = sumSlow as + sumSlow bs := by
  intro as
  induction as with
  | nil =>
      intro bs
  ...
  | cons a as ih =>
      intro bs
      simp [appendSlow, sumSlow, ih, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
have sum_rev : ∀ as : List Nat, sumSlow (revSlow as) = sumSlow as := by
  intro as
  induction as with
  | nil =>
      simp [revSlow, sumSlow]
...
### challenge_v6_mutual_depth_stats_mirror
...
induction tree with
| leaf =>
    simp [mirror, evenDepthStats, oddDepthStats]
| node left value right ihLeft ihRight =>
    simp [mirror, evenDepthStats, oddDepthStats, ihLeft, ihRight, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
```

## V25 step-prefix exact-label ablation

This section is prompt guidance only. These labels do **not** appear as theorem hypotheses in the v25 templates, and they carry no Lean proof term. Use them only to choose the proof shape; the fixed theorem template still cannot be changed.

V25 preserves the exact semantic roots from the v18 detailed labels, but replaces the original planning prefix with `step_`. This tests whether any code-like prefix attached to the exact roots is enough, or whether the original v18 prefixed form is special.

Detailed skeleton labels to keep in mind:

- `step_induct_on_tree`: the recursive structure to inspect is the `NatTree` argument.
- `step_leaf_simp_definitions`: the `leaf` case should reduce by unfolding the local tree/list definitions.
- `step_node_by_cases_value_eq_target`: the `node` case should separate whether `value = target` before simplifying keep/drop/count behavior.
- `step_simp_mirror_flatten_keep_drop_append`: after the case split, simplify mirror/flatten/keep/drop/list metrics using the append/filter helper hypotheses and both subtree induction hypotheses.
- `step_normalize_nat_add_assoc_comm_left_comm`: normalize additive metric expressions with associativity, commutativity, and left-commutativity.
- `step_finish_with_metric_packers`: finish any remaining conjunction packaging using the available pair/top or named metric packers when needed.

Do not emit these labels as theorem assumptions; they are not in scope for v25. The final answer must still be only the Lean proof body for `{{PROOF}}` and must pass Lean unchanged.
