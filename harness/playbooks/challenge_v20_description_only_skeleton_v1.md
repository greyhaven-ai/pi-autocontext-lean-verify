...
Prompt-only proof skeleton. These reminders are not Lean hypotheses or names in the theorem. Do not emit them as identifiers; use them only to choose the proof shape.

- Induct on the `NatTree` argument.
- In the leaf case, simplify by unfolding the local tree and list metric definitions.
- In the node case, split on whether the node value equals the target before simplifying keep/drop/count behavior.
- After the case split, simplify mirror, flatten, keep/drop filters, and list metrics with the append/filter helper hypotheses and both subtree induction hypotheses.
- Normalize natural-number addition with associativity, commutativity, and left-commutativity.
- Finish any remaining conjunction packaging with the available pair/top or named metric packers.
