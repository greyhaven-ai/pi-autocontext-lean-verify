...
/-- Challenge v11: apply a bundled keep-side metric proof to statsAcc equality. -/
theorem keepStatsAcc_from_metric_bundle
    (target : Nat) (tree : NatTree) (hits total sum : Nat)
    (hMetrics : countSlow target (keepEqSlow target (flatten (mirror tree))) =
