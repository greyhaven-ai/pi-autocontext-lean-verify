...
def default_seed_playbook(group: str) -> Path:
    if group in {"challenge_v3_generalization", "challenge_transfer"}:
...
def main() -> int:
    ...
    run_root = Path(args.run_root) if args.run_root else ROOT / "results" / f"{utc_stamp()}_proof_transfer_benchmark_{args.fixture_group}"
