...
/-- Challenge v10: reify the three-field stats accumulator into count/length/sum metrics. -/
theorem statsAcc_reify_list
    (target : Nat) (xs : List Nat) (hits total sum : Nat) :
    statsAcc target xs hits total sum =
      (countSlow target xs + hits, lengthSlow xs + total, sumSlow xs + sum) := by
{{PROOF}}
