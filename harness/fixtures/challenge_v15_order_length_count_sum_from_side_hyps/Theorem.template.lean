...
/-- Challenge v15: reassemble length/count/sum metric-order conclusion from side-grouped keep/drop hypotheses. -/
theorem orderLengthCountSum_from_side_hyps (target : Nat) (tree : NatTree)
    (hKeep : lengthSlow (keepEqSlow target (flatten (mirror tree))) =
    ...
    (hDrop : lengthSlow (dropEqSlow target (flatten (mirror tree))) =
