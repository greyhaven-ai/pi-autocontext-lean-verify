...
/-- Challenge v14: simultaneous keep/drop metric bundle through tree mirror, metric order count_sum_length. -/
theorem partitionMetricsOrderCountSumLength_flatten_mirror (target : Nat) (tree : NatTree) :
    (countSlow target (keepEqSlow target (flatten (mirror tree))) =
      ...
      lengthSlow (dropEqSlow target (flatten (mirror tree))) =
        lengthSlow (dropEqSlow target (flatten tree))) := by
{{PROOF}}
