...
/-- Challenge v11: apply a bundled drop-side metric proof to statsAcc equality. -/
theorem dropStatsAcc_from_metric_bundle
    (target : Nat) (tree : NatTree) (hits total sum : Nat)
    (hMetrics : countSlow target (dropEqSlow target (flatten (mirror tree))) =
