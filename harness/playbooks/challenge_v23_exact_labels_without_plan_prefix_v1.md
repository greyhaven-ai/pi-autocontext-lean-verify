...
## V23 exact labels without detailed-plan prefix

This section is prompt guidance only. These labels do **not** appear as theorem hypotheses in the v23 templates, and they carry no Lean proof term. Use them only to choose the proof shape; the fixed theorem template still cannot be changed.

Detailed skeleton labels to keep in mind:

- `induct_on_tree`: the recursive structure to inspect is the `NatTree` argument.
- `leaf_simp_definitions`: the `leaf` case should reduce by unfolding the local tree/list definitions.
- `node_by_cases_value_eq_target`: the `node` case should separate whether `value = target` before simplifying keep/drop/count behavior.
- `simp_mirror_flatten_keep_drop_append`: after the case split, simplify mirror/flatten/keep/drop/list metrics using the append/filter helper hypotheses and both subtree induction hypotheses.
- `normalize_nat_add_assoc_comm_left_comm`: normalize additive metric expressions with associativity, commutativity, and left-commutativity.
- `finish_with_metric_packers`: finish any remaining conjunction packaging using the available pair/top or named metric packers when needed.

Do not emit these labels as theorem assumptions; they are not in scope for v23. The final answer must still be only the Lean proof body for `{{PROOF}}` and must pass Lean unchanged.
