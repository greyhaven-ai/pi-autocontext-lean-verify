...
## Prompt-only v18 detailed proof-plan skeleton

This section is prompt guidance only. These names do **not** appear as theorem hypotheses in the v18 templates, and they carry no Lean proof term. Use them only to choose the proof shape; the fixed theorem template still cannot be changed.

Detailed skeleton names to keep in mind:

- `plan_induct_on_tree`: the recursive structure to inspect is the `NatTree` argument.
- `plan_leaf_simp_definitions`: the `leaf` case should reduce by unfolding the local tree/list definitions.
- `plan_node_by_cases_value_eq_target`: the `node` case should separate whether `value = target` before simplifying keep/drop/count behavior.
- `plan_simp_mirror_flatten_keep_drop_append`: after the case split, simplify mirror/flatten/keep/drop/list metrics using the append/filter helper hypotheses and both subtree induction hypotheses.
- `plan_normalize_nat_add_assoc_comm_left_comm`: normalize additive metric expressions with associativity, commutativity, and left-commutativity.
- `plan_finish_with_metric_packers`: finish any remaining conjunction packaging using the available pair/top or named metric packers when needed.

Do not emit these names as theorem assumptions; they are not in scope for v18. The final answer must still be only the Lean proof body for `{{PROOF}}` and must pass Lean unchanged.
