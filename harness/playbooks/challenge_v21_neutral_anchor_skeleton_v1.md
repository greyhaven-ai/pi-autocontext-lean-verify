...
Prompt-only proof skeleton with neutral step anchors. These anchors are not Lean hypotheses and are not theorem identifiers. Use them only to choose the proof shape; do not emit the anchor text as code.

- Tree induction: induct on the `NatTree` argument so the two subtree induction hypotheses are available in the node case.
- Leaf simplification: in the leaf case, simplify by unfolding the local tree and list metric definitions.
- Node equality split: in the node case, split on whether the node value equals the target before simplifying keep/drop/count behavior.
- Append/filter simplification: after the split, simplify mirror, flatten, keep/drop filters, and list metrics with the append/filter helper hypotheses and both subtree induction hypotheses.
- Nat addition normalization: normalize natural-number addition with associativity, commutativity, and left-commutativity.
- Packer finish: finish any remaining conjunction packaging with the available pair/top or named metric packers.
