...
`0.1.12` was published through GitHub trusted publishing from tag `v0.1.12` (workflow `25761371690`), is npm `latest`, has two npm provenance attestations, and was registry-smoked with setup `1/1` over 120 fixtures plus a focused v15 append/filter-hypothesis attribution contrast: seeded `1/1`, unseeded `0/1`, direct `0/1`. The source tree after `0.1.12` adds v16/v17 diagnostics; v17 detailed proof-plan hints are promising, but a future release should wait for an explicit package decision and registry smoke plan.
