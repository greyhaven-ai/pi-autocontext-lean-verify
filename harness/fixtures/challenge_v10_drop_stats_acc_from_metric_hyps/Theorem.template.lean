...
/-- Challenge v10: drop-side stats accumulator equality from already-proved metric equalities. -/
theorem dropStatsAcc_from_metric_hyps
    (target : Nat) (tree : NatTree) (hits total sum : Nat)
    (hCount : countSlow target (dropEqSlow target (flatten (mirror tree))) =
        countSlow target (dropEqSlow target (flatten tree)))
    (hLen : lengthSlow (dropEqSlow target (flatten (mirror tree))) =
        lengthSlow (dropEqSlow target (flatten tree)))
    (hSum : sumSlow (dropEqSlow target (flatten (mirror tree))) =
