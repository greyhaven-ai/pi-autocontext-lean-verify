...
/-- Challenge v6: partition/count invariants through mirrored tree flattening. -/
theorem partition_flatten_mirror_stats (target : Nat) (tree : NatTree) :
    countSlow target (keepEqSlow target (flatten (mirror tree))) = countSlow target (flatten tree) ∧
      lengthSlow (keepEqSlow target (flatten (mirror tree))) +
          lengthSlow (dropEqSlow target (flatten (mirror tree))) =
        lengthSlow (flatten tree) := by
{{PROOF}}
