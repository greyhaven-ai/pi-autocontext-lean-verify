...
/-- Challenge v10: stats accumulator extensionality from equal count/length/sum metrics. -/
theorem statsAcc_extensional_from_metrics
    (target : Nat) (xs ys : List Nat) (hits total sum : Nat)
    (hCount : countSlow target xs = countSlow target ys)
    (hLen : lengthSlow xs = lengthSlow ys)
    (hSum : sumSlow xs = sumSlow ys) :
    statsAcc target xs hits total sum = statsAcc target ys hits total sum := by
{{PROOF}}
