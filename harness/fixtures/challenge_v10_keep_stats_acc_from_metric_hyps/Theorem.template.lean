...
/-- Challenge v10: keep-side stats accumulator equality from already-proved metric equalities. -/
theorem keepStatsAcc_from_metric_hyps
    (target : Nat) (tree : NatTree) (hits total sum : Nat)
    (hCount : countSlow target (keepEqSlow target (flatten (mirror tree))) =
        countSlow target (keepEqSlow target (flatten tree)))
    (hLen : lengthSlow (keepEqSlow target (flatten (mirror tree))) =
        lengthSlow (keepEqSlow target (flatten tree)))
    (hSum : sumSlow (keepEqSlow target (flatten (mirror tree))) =
        sumSlow (keepEqSlow target (flatten tree))) :
    statsAcc target (keepEqSlow target (flatten (mirror tree))) hits total sum =
      statsAcc target (keepEqSlow target (flatten tree)) hits total sum := by
{{PROOF}}
