...
`0.1.15` was published through GitHub trusted publishing from tag `v0.1.15` (workflow `25838541595`), is npm `latest`, has two npm provenance attestations, and includes the process-group timeout/short-run-root hardening required before v22 prompt-transfer stress tests. Registry smoke covered preflight, setup `1/1`, default `action="run"` short temp-root behavior, and packaged `process_utils.py` process-group cleanup. The source tree after `0.1.15` adds v22 code-like snake_case anchor fixtures and v23 exact-label-minus-prefix fixtures for prompt-transfer ablations.
