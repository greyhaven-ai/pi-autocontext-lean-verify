...
/-- Challenge v9 gradient: keep-side partition length through tree mirror. -/
theorem keepLength_flatten_mirror (target : Nat) (tree : NatTree) :
    lengthSlow (keepEqSlow target (flatten (mirror tree))) =
      lengthSlow (keepEqSlow target (flatten tree)) := by
{{PROOF}}
