...
/-- Challenge v8 diagnostic: tree mirror invariance for the raw three-field stats accumulator, before partitioning. -/
theorem statsAcc_flatten_mirror
    (target : Nat) (tree : NatTree) (hits total sum : Nat) :
    statsAcc target (flatten (mirror tree)) hits total sum =
